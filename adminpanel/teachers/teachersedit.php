<?php
	...
	if($_SERVER['REQUEST_METHOD'] == 'GET'){
		...
		do{
            if(empty($id) || empty($name) || empty($email) || empty($phone) || empty($course) || empty($file)){
                ...
                if($filetype == "png" || $filetype == "jpg" || $filetype == "jpeg"){
                    if(move_uploaded_file($_FILES['image']['tmp_name'],$target_file)){
                        $sql = mysqli_query($db,"UPDATE `teachers` SET `name`='$name', `email`='$email', `phone`='$phone', `course`='$course', `image`='$file', `teacherid`='$teacherid' WHERE id=$id");
                        if($sql){
                            $successmsg = "You have successfully Updated a Teacher";
                            header("Location: teachers.php");
                            exit;
                        } 
                        else{
