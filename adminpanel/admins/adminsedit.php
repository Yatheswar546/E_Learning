<?php
	...
	if($_SERVER['REQUEST_METHOD'] == 'GET'){
		...
		$id = $_GET["id"];
        // Read the Data
        $admins = mysqli_query($db,"SELECT * FROM `admins` WHERE id=$id");
		...
		do{
			if(empty($id) || empty($name) || empty($email) || empty($phone) || empty($password) || empty($file)){
				...
				if($filetype == "png" || $filetype == "jpg" || $filetype == "jpeg"){
					if(move_uploaded_file($_FILES['image']['tmp_name'],$target_file)){
						$sql = mysqli_query($db,"UPDATE `admins` SET `name`='$name',`email`='$email',`image`='$file',`phone`='$phone' WHERE id='$id'");
						if($sql){
							header("Location: admins.php");
							exit;
						} 
